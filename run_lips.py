...
def play(arg, path = None):
    ...
    policy = ActorCritic(env.num_obs,
                         env.num_privileged_obs,
                         env.num_obs_history,env.num_actions,
                         inference_stage=train_cfg.runner.stage,
...
if __name__ == '__main__':
    args = get_args()
    if args.play:
        path = "logs/Lips/Oct12_18-21-16_noraml/model_5000.pt"
        play(args,path)
        exit()
    else:
        env, runner , env_cfg ,train_cfg = launch(args)
        runner.load("logs/Lips/Oct12_18-21-16_noraml/model_5000.pt")
