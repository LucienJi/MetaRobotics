from LowLevelController.modules.ac import ActorCritic
import torch
import torch.nn as nn
import torch.optim as optim
import torch.nn.functional as F
import numpy as np
from LowLevelController.configs.training_config import RunnerCfg
from LowLevelController.buffers.storage import RolloutStorage
...
class PPO:
    ...
    def update(self):
        ...
        for obs_batch, critic_observations_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, \
                       old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (hid_a_batch, hid_e_batch) in generator:

            self.actor_critic.act_expert(obs_batch, privileged_obs_batch, obs_history_batch,
                                         hidden_states_e= None, hidden_states_a= hid_a_batch)
            ...
            for epoch in range(self.cfg.num_adaptation_module_substeps):
                ...
                adaptation_pred = self.actor_critic.get_student_latent(obs_batch,privileged_obs_batch,obs_history_batch,
                                                                       masks_batch,hidden_states_e=hid_e_batch,
