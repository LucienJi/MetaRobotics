...
import torch
from utils.torch_utils import split_and_pad_trajectories
class RolloutStorage:
    ...
    def _save_hidden_states(self, hidden_states):
        ...
        hid_e = hidden_states[1] if isinstance(hidden_states[1], tuple) else (hidden_states[1],)

        # initialize if needed 
        #! 注意, hid_a 是 (layer, bz, hidden_dim) 的形式, 在我们这里就是, (1,n_env, hidden_dim)
    ...
    def reccurent_mini_batch_generator(self,num_mini_batches, num_epochs=8):
        ...
        padded_pri_obs_trajectories, trajectory_masks = split_and_pad_trajectories(self.privileged_observations, self.dones)
        ...
        for ep in range(num_epochs):
            ...
            for i in range(num_mini_batches):
                ...
                privileged_obs_batch = padded_pri_obs_trajectories[:, first_traj:last_traj]
                
                obs_history_batch = self.observation_histories[:, start:stop]
                # privileged_obs_batch = self.privileged_observations[:, start:stop]
                raw_obs_batch = critic_observations[:, start:stop]
                ...
                hid_e_batch = [ saved_hidden_states.permute(2, 0, 1, 3)[last_was_done][first_traj:last_traj].transpose(1, 0).contiguous()
                ...
                yield obs_batch, raw_obs_batch,privileged_obs_batch,obs_history_batch, actions_batch, values_batch, advantages_batch, returns_batch, \
                       old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (hid_a_batch, hid_e_batch), masks_batch
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        saved_hidden_states_a = [hid_a.permute(0, 2, 1,3).flatten(0,1) for hid_a in saved_hidden_states_a]
        saved_hidden_states_e = [hid_e.permute(0, 2, 1,3).flatten(0,1) for hid_e in saved_hidden_states_e] #(bz, 1, hidden_dim)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                hid_a_batch = [hid_a[batch_idx].transpose(0,1) for hid_a in saved_hidden_states_a]
                hid_e_batch = [hid_e[batch_idx].transpose(0,1) for hid_e in saved_hidden_states_e]
                # remove the tuple for GRU
                hid_a_batch = hid_a_batch[0] if len(hid_a_batch)==1 else hid_a_batch
                hid_e_batch = hid_e_batch[0] if len(hid_e_batch)==1 else hid_e_batch
                hid_a_batch.reshape()
                ...
                yield obs_batch, critic_observations_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, \
                       old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, (hid_a_batch, hid_e_batch)
