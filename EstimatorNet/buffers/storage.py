...
class RolloutStorage:
    ...
    def __init__(self, num_envs, num_transitions_per_env, obs_shape, privileged_obs_shape, obs_history_shape, actions_shape, device='cpu'):
        ...
        self.foot_height = torch.zeros(num_transitions_per_env, num_envs, 4, device=self.device)
        self.contact = torch.zeros(num_transitions_per_env, num_envs, 4, device=self.device)
    ...
    def add_transitions(self, transition: Transition):
        ...
        self.foot_height[self.step].copy_(transition.foot_height)
        self.contact[self.step].copy_(transition.contact)
        self.step += 1
    ...
    def mini_batch_generator(self, num_mini_batches, num_epochs=8):
        ...
        foot_height = self.foot_height.flatten(0, 1)
        contact = self.contact.flatten(0, 1)
        ...
        for epoch in range(num_epochs):
            for i in range(num_mini_batches):
                ...
                foot_height_batch = foot_height[batch_idx]
                contact_batch = contact[batch_idx]
                ...
                yield obs_batch, critic_observations_batch, privileged_obs_batch, obs_history_batch, actions_batch, target_values_batch, advantages_batch, returns_batch, \
                       old_actions_log_prob_batch, old_mu_batch, old_sigma_batch, env_bins_batch,base_vel_batch,dones_batch,foot_height_batch,contact_batch
