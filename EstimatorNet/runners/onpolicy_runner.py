...
from EstimatorNet.algorithms.ppo import PPO  
import torch 
import numpy as np 
...
class Runner:
    ...
    def learn(self, num_learning_iterations):
        ...
        for it in range(self.current_learning_iteration, tot_iter):
            ...
            mean_value_loss, mean_surrogate_loss,mean_entropy_loss,\
                 mean_foot_height_loss, mean_vel_loss, mean_contact_loss = self.alg.update()
    ...
    def log(self, locs, width=80, pad=35):
        ...
        self.writer.add_scalar('Loss/Foot_Height', locs['mean_foot_height_loss'], locs['it'])
        self.writer.add_scalar('Loss/Vel_estimation', locs['mean_vel_loss'], locs['it'])
        self.writer.add_scalar('Loss/Contact', locs['mean_contact_loss'], locs['it'])
        ...
        if len(locs['rewbuffer']) > 0:
            log_string = (f"""{'#' * width}\n"""
                          ...
                          f"""{'Foot Height loss:':>{pad}} {locs['mean_foot_height_loss']:.4f}\n"""
                          f"""{'Vel Est loss:':>{pad}} {locs['mean_vel_loss']:.4f}\n"""
                          f"""{'Contact loss:':>{pad}} {locs['mean_contact_loss']:.4f}\n"""
